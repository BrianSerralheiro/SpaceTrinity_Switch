...
public class Squid : EnemyBase {
	...
	void Punch(){
		...
		for (int i = 1; i < tentacleSize; i++)
		{
			if(side!=0 || timer>1)tentacle[0,i].localRotation=Quaternion.RotateTowards(tentacle[0,i].localRotation,Quaternion.Euler(0,0,-45),Time.deltaTime*90);
			else tentacle[0,i].localRotation=Quaternion.RotateTowards(tentacle[0,i].localRotation,Quaternion.Euler(0,0,15),Time.deltaTime*90);
			if(side!=1 || timer>1)tentacle[1,i].localRotation=Quaternion.RotateTowards(tentacle[1,i].localRotation,Quaternion.Euler(0,0,45),Time.deltaTime*90);
			else tentacle[1,i].localRotation=Quaternion.RotateTowards(tentacle[1,i].localRotation,Quaternion.Euler(0,0,-15),Time.deltaTime*90);
		}
		if(timer<0){
			update=Wait;
			timer=3;
	...
	void Load(){
		...
		for (int i = 0; i < missiles.Length; i++)
		{
			...
			go.transform.position=claws[side].position+(side==0?offsetX:-offsetX)+offsetY*((i-1)*2)+Vector3.forward*0.1f;
