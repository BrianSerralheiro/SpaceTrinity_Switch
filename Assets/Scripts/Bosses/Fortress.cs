...
public class Fortress : EnemyBase {
    ...
    int turretID, shotId, trailID, impactID, dirX, dirY;
    bool flag;
    ...
    public override void SetSprites (EnemyInfo ei) {
        ...
        for (int j = 0; j < 2; j++) {
            ...
            for (int i = 0; i < 2; i++) {
                g = new GameObject ("treadpiece" + i);
                g.transform.localScale = new Vector3 (transform.localScale.x, -5, 1);
    ...
    void Dying () {
        ...
        if (transform.position.y < -Scaler.sizeY / 2) Loader.Scene ("Secret");
    ...
    void Movement () {
        ...
        if (transform.position.x > 10) dirX = Random.Range (-2, -4);
        if (transform.position.y < 1) dirY = Random.Range (2, 4);
